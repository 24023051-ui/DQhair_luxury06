  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.error("Firestore user profile creation failed", dbErr);
        navigate('/');
      }
    } catch (err: any) {
    ...
    <div className="bg-[#0A0A0A] min-h-screen flex items-center justify-center pt-24 pb-12 px-4 relative overflow-hidden">
      ...
      >
         <div className="text-center mb-10">
          <span className="text-[#C9A84C] text-[10px] uppercase tracking-[0.4em] font-bold mb-3 block">Create Account</span>
          <h2 className="font-display text-4xl italic text-white flex justify-center items-center gap-2">
            Join <span className="not-italic font-sans font-black">Us.</span>
          </h2>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleRegister}>
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full py-4 mt-8 bg-black border border-[#C9A84C] text-[#C9A84C] font-bold tracking-[0.2em] uppercase text-[11px] rounded-sm hover:-translate-y-1 hover:bg-[#C9A84C] hover:text-black transition-all shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div className="mt-8 flex items-center justify-center gap-4">
          ...
          <span className="text-[9px] uppercase tracking-widest text-[#A0A0A0]">Or join with</span>
          <div className="flex-1 h-[1px] bg-white/5"></div>
        </div>
        ...
        <div className="mt-10 pt-8 border-t border-white/5 text-center text-[10px] uppercase tracking-widest text-[#A0A0A0]">
          Already have an account? <Link to="/login" className="text-[#C9A84C] font-bold hover:text-[#F0D080] transition-colors ml-1 border-b border-[#C9A84C]/50 hover:border-[#F0D080] pb-0.5">Sign In</Link>
        </div>
      </motion.div>
    </div>
  );
